...
public static class PawnUpgrade extends Move {
		
		final Move decoratedMove;
		final Pawn upgradedPawn;
		
		public PawnUpgrade(final Move decoratedMove) {
			super(decoratedMove.getBoard(), decoratedMove.getMovedPiece(), decoratedMove.getDestinationCoordinate());
			this.decoratedMove = decoratedMove;
			this.upgradedPawn = (Pawn) decoratedMove.getMovedPiece();
		}
		...
		public Board execute() {
			...
			final Builder builder	= new Builder();
			for(final Piece piece : pawnMoveBoard.currentPlayer().getActivePieces()) {
				if(!this.upgradedPawn.equals(piece)) {
					builder.setPiece(piece);
				}
			}
			for(final Piece piece : pawnMoveBoard.currentPlayer().getOpponent().getActivePieces()) {
			...
			builder.setPiece(this.upgradedPawn.getUpgradePiece().movePiece(this));
			builder.setMoveMaker(pawnMoveBoard.currentPlayer().getAlliance());
			return builder.build();
		}
		...
		@Override
		public int hashCode() {
			return decoratedMove.hashCode() + (31 * upgradedPawn.hashCode());
		}
		
		@Override
		public boolean equals(final Object other) {
			return this == other || other instanceof PawnUpgrade && (super.equals(other));
		}
	}
	...
	static abstract class CastleMove extends Move // class for Major Move (just a normal move)
	{
		...
		public boolean equals(final Object other) {
			...
			final CastleMove otherCasteMove = (CastleMove)other;
			return super.equals(otherCasteMove) && this.castleRook.equals(otherCasteMove.getCastleRook());
		}
	}
